            ...
            for exit_dir, next_room in current_map[room].items():
                if next_room == '?':
                    ...
                    if temp_player.current_room.id not in current_map:
                        ...
                        current_map[temp_player.current_room.id][opposite[exit_dir]] = room_id
            if len(options) > 0:
...
def get_next_move(player, curr_map):
    """
    Return next move or series of move
    """
    options = []
...
def get_traversal_path(player):
    """
    Find and return list of directions to traverse map
    """
...
path_list = [[]] * 100
for i in range(100):
    path_list[i] = get_traversal_path(player)
